...
from apps.authentication.models import User, RolePermission

@pytest.mark.django_db
class TestSecurity:
    def test_unauthenticated_access_redirects_to_login(self, client):
        """Test: unauthenticated access redirects to login"""
        response = client.get(reverse('company:dashboard'))
    ...
    def test_csrf_token_required_for_post(self, client, user):
        """Test: CSRF token required for POST requests"""
        client.force_login(user)
        client.enforce_csrf_checks = True
        
        response = client.post(reverse('crm:customer_create'), {"name": "Test"})
