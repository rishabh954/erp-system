...
def test_multitenant_boundary(client, user_a, company_a, company_b):
    ...
    # 2. Lead (CRM)
    lead_b = Lead.objects.create(company=company_b, name="Lead B", expected_revenue=0)
    ...
    # 4. Vendor (Purchase)
    vend_b = Vendor.objects.create(company=company_b, name="Vendor B")
    ...
    # 6. Project
    proj_b = Project.objects.create(company=company_b, name="Proj B")
    res = client.get(reverse("projects:detail", kwargs={"pk": proj_b.pk}))
    assert res.status_code == 404
    ...
    # 8. Employee (HRMS)
    dept_b = Department.objects.create(company=company_b, name="Dept B", code="DB")
