            ...
            if hasattr(pattern, 'url_patterns'):
                ...
                if model:
                    ...
                    if is_uuid and int_pattern.search(full_pattern):
                        errors.append(f"URL {full_pattern} uses <int:pk> but model {model.__name__} has a UUID PK.")
                    elif not is_uuid and uuid_pattern.search(full_pattern):
                        errors.append(f"URL {full_pattern} uses <uuid:pk> but model {model.__name__} has an int PK.")
...
def test_template_field_references():
    ...
    model_mapping = {
        'order': ['sales.SalesOrder', 'purchase.PurchaseOrder', 'manufacturing.ManufacturingOrder'],
        'sales_order': ['sales.SalesOrder'],
        'purchase_order': ['purchase.PurchaseOrder'],
        'bill': ['purchase.Bill'],
        'invoice': ['sales.Invoice'],
        'customer': ['crm.Customer', 'authentication.User'], # User is often customer portal user
    ...
    allowed_methods = ['pk', 'id', 'get_status_display', 'get_priority_display', 
                       'get_method_display', 'get_movement_type_display', 
                       'get_allocation_method_display', 'get_cost_type_display',
                       'get_full_name']
        ...
        for var_name, field_name in matches:
            if var_name in resolved_models:
                ...
                if not is_valid:
                    ...
                    errors.append(f"Template {rel_path}: '{var_name}.{field_name}' referenced, but '{field_name}' not found on {var_name} models {[m.__name__ for m in models_to_check]}.")
