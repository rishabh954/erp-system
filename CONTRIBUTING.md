...
Never commit secrets, passwords, API keys, or active credentials into version control. 
- Use the `.env.example` file to document environment variables, but ensure it only contains placeholders or dummy values.
- **Immediate Rotation Rule**: If a secret is accidentally leaked in the codebase, it must be considered permanently compromised. You must immediately rotate the credential in the underlying service (e.g., revoke the API key, change the email password, or update the database password) and update the production deployment configurations. Do NOT just remove it from the file; it must be invalidated at the source.
