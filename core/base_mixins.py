...
class CompanyScopedMixin:
    ...
    def get_queryset(self):
        ...
        if (
            hasattr(self.request.user, "primary_company")
            and self.request.user.primary_company
        ):
...
class AuditCreateMixin:
    ...
    def form_valid(self, form):
        if hasattr(form.instance, "company_id") and hasattr(
            self.request.user, "primary_company"
        ):
...
class RolePermissionMixin(UserPassesTestMixin):
    ...
    def test_func(self):
        ...
        if self.required_module and self.required_action:
            if not user.has_module_permission(
                self.required_module, self.required_action
            ):
                return False
