...
from apps.authentication.models import (
    ...
    ActivityLog, UserSession, PasswordResetToken,
)

@admin.register(User)
class UserAdmin(BaseUserAdmin):
    list_display   = ['email', 'full_name', 'role', 'primary_company', 'is_active', 'date_joined']
    ...
    fieldsets = (
        ...
        (_('Preferences'),       {'fields': ('language', 'timezone', 'theme', 'date_format')}),
        (_('Security'),          {'fields': ('two_factor_enabled', 'two_factor_method',
                                              'failed_login_attempts', 'locked_until')}),
        (_('Permissions'),       {'fields': ('is_active', 'is_staff', 'is_superuser',
                                              'groups', 'user_permissions')}),
        (_('Activity'),          {'fields': ('last_login', 'last_active', 'last_login_ip', 'date_joined')}),
    )
    add_fieldsets = (
        (None, {'classes': ('wide',),
...
class ActivityLogAdmin(admin.ModelAdmin):
    list_display  = ['user', 'action', 'module', 'resource_type', 'ip_address', 'created_at']
    list_filter   = ['action', 'module', 'company']
    search_fields = ['user__email', 'description', 'resource_id']
    readonly_fields = ['user', 'company', 'action', 'module', 'resource_type',
...
class ModulePermissionAdmin(admin.ModelAdmin):
    list_display = ['role', 'module', 'can_create', 'can_read', 'can_update',
                    'can_delete', 'can_approve', 'can_export']
    list_filter  = ['role', 'module']
    list_editable = ['can_create', 'can_read', 'can_update', 'can_delete', 'can_approve', 'can_export']
...
from apps.company.models import (
    ...
    ExchangeRate, FiscalYear, Tax, TaxGroup,
)

@admin.register(Company)
class CompanyAdmin(admin.ModelAdmin):
    list_display  = ['name', 'status', 'subscription_plan', 'default_currency', 'created_at']
    list_filter   = ['status', 'subscription_plan', 'company_type']
    search_fields = ['name', 'legal_name', 'tax_id']
    readonly_fields = ['created_at', 'updated_at']

    fieldsets = (
        ('Basic Info',    {'fields': ('name', 'legal_name', 'registration_number',
                                       'tax_id', 'vat_number', 'company_type', 'industry', 'status')}),
        ...
        ('Settings',      {'fields': ('language', 'timezone', 'date_format', 'number_format')}),
...
from apps.hrms.models import (
    Employee, JobTitle, Attendance, LeaveType,
    LeaveRequest, LeaveBalance, PayrollPeriod, Payslip,
)

@admin.register(Employee)
class EmployeeAdmin(admin.ModelAdmin):
    list_display  = ['employee_id', 'full_name', 'company', 'department', 'job_title', 'status', 'joining_date']
...
class LeaveRequestAdmin(admin.ModelAdmin):
    list_display  = ['number', 'employee', 'leave_type', 'start_date', 'end_date', 'total_days', 'status']
...
class PayrollPeriodAdmin(admin.ModelAdmin):
    list_display  = ['name', 'company', 'period_start', 'period_end', 'status', 'total_net']
    list_filter   = ['company', 'status']


@admin.register(Payslip)
class PayslipAdmin(admin.ModelAdmin):
    list_display  = ['number', 'employee', 'payroll_period', 'gross_salary', 'net_salary', 'status']
...
class LeadAdmin(admin.ModelAdmin):
    list_display  = ['number', 'name', 'company_name', 'status', 'source', 'expected_revenue', 'assigned_to']
...
class CustomerAdmin(admin.ModelAdmin):
    list_display  = ['name', 'customer_code', 'customer_type', 'email', 'phone', 'is_active']
...
from apps.sales.models import Quotation, SalesOrder, Invoice, Payment
...
class InvoiceAdmin(admin.ModelAdmin):
    list_display  = ['number', 'customer', 'invoice_date', 'due_date', 'total', 'balance_due', 'status']
    list_filter   = ['company', 'status']
    search_fields = ['number', 'customer__name']
...
class PaymentAdmin(admin.ModelAdmin):
    list_display  = ['number', 'invoice', 'customer', 'amount', 'payment_date', 'method', 'status']
    list_filter   = ['company', 'status', 'method']


# ─── Purchase ──────────────────────────────────────────────────────────────────

from apps.purchase.models import Vendor, PurchaseRequest, PurchaseOrder
...
from apps.inventory.models import (
    Product, ProductCategory, Warehouse, StockRecord, StockMovement,
)
...
class StockRecordAdmin(admin.ModelAdmin):
    list_display  = ['product', 'warehouse', 'quantity_on_hand', 'quantity_reserved', 'average_cost']
...
class StockMovementAdmin(admin.ModelAdmin):
    list_display  = ['number', 'product', 'warehouse', 'movement_type', 'quantity', 'movement_date']
...
from apps.accounting.models import Account, Journal, JournalEntry, BankAccount
...
class JournalEntryAdmin(admin.ModelAdmin):
    list_display  = ['number', 'journal', 'date', 'total_debit', 'total_credit', 'status']
...
class ProjectAdmin(admin.ModelAdmin):
    list_display  = ['number', 'name', 'status', 'priority', 'manager', 'start_date', 'end_date']
...
class TicketAdmin(admin.ModelAdmin):
    list_display  = ['number', 'title', 'status', 'priority', 'assigned_to', 'sla_breached', 'created_at']
...
class EmailLogAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ['recipient_email', 'recipient_name', 'subject', 'body',
                       'status', 'sent_at', 'error_message', 'template', 'created_at']


# --- Automatically Generated Admin Registrations ---

from apps.authentication.models import Role, Permission, PasswordResetToken, EmailVerificationToken, UserSession
...
class UserSessionAdmin(admin.ModelAdmin):
    list_display = ['user', 'session_key', 'ip_address', 'user_agent', 'device', 'location']

from apps.company.models import ExchangeRate, TaxGroup, CompanySettings
...
from apps.hrms.models import JobTitle, EmployeeDocument, EmployeeSkill, ExperienceRecord, WorkSchedule, LeaveType, LeaveBalance, SalaryStructure, SalaryComponent, EmployeeSalary

@admin.register(JobTitle)
class JobTitleAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'name', 'description', 'grade']

@admin.register(EmployeeDocument)
class EmployeeDocumentAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'employee', 'document_type', 'title']
...
class WorkScheduleAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'name', 'check_in_time', 'check_out_time']

@admin.register(LeaveType)
class LeaveTypeAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'name', 'code', 'days_allowed']

@admin.register(LeaveBalance)
class LeaveBalanceAdmin(admin.ModelAdmin):
    list_display = ['employee', 'leave_type', 'year', 'allocated', 'used', 'carried_forward']

@admin.register(SalaryStructure)
class SalaryStructureAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'name', 'description', 'is_active']

@admin.register(SalaryComponent)
class SalaryComponentAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'salary_structure', 'name', 'code']

@admin.register(EmployeeSalary)
class EmployeeSalaryAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'employee', 'salary_structure', 'basic_salary']

from apps.crm.models import LeadActivity

@admin.register(LeadActivity)
class LeadActivityAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'lead', 'activity_type', 'subject']

from apps.sales.models import QuotationLine, SalesOrder, SalesOrderLine, InvoiceLine

@admin.register(QuotationLine)
class QuotationLineAdmin(admin.ModelAdmin):
    list_display = ['quotation', 'product', 'description', 'quantity', 'unit_price', 'discount_percent']

@admin.register(SalesOrder)
class SalesOrderAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'currency', 'exchange_rate', 'notes']

@admin.register(SalesOrderLine)
class SalesOrderLineAdmin(admin.ModelAdmin):
    list_display = ['sales_order', 'product', 'description', 'quantity', 'unit_price', 'discount_percent']

@admin.register(InvoiceLine)
class InvoiceLineAdmin(admin.ModelAdmin):
    list_display = ['invoice', 'product', 'description', 'quantity', 'unit_price', 'discount_percent']

from apps.purchase.models import PurchaseRequest, PurchaseRequestLine, PurchaseOrderLine, GoodsReceipt, GoodsReceiptLine
...
class PurchaseRequestLineAdmin(admin.ModelAdmin):
    list_display = ['request', 'product', 'description', 'quantity', 'unit', 'estimated_unit_price']

@admin.register(PurchaseOrderLine)
class PurchaseOrderLineAdmin(admin.ModelAdmin):
    list_display = ['purchase_order', 'product', 'description', 'quantity', 'unit', 'unit_price']

@admin.register(GoodsReceipt)
class GoodsReceiptAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'notes', 'number', 'purchase_order']

@admin.register(GoodsReceiptLine)
class GoodsReceiptLineAdmin(admin.ModelAdmin):
    list_display = ['goods_receipt', 'po_line', 'quantity_received', 'quantity_accepted', 'quantity_rejected', 'rejection_reason']

from apps.inventory.models import UnitOfMeasure, Brand, ProductVariant, BinLocation, InventoryTransfer, InventoryTransferLine

@admin.register(UnitOfMeasure)
class UnitOfMeasureAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'name', 'abbreviation', 'uom_type']
...
class InventoryTransferAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'notes', 'number', 'from_warehouse']

@admin.register(InventoryTransferLine)
class InventoryTransferLineAdmin(admin.ModelAdmin):
    list_display = ['transfer', 'product', 'variant', 'quantity_requested', 'quantity_sent', 'quantity_received']

from apps.accounting.models import Journal, JournalItem, BankTransaction, TaxReturn

@admin.register(Journal)
class JournalAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'name', 'code', 'journal_type']

@admin.register(JournalItem)
class JournalItemAdmin(admin.ModelAdmin):
    list_display = ['journal_entry', 'account', 'description', 'debit', 'credit', 'currency']

@admin.register(BankTransaction)
class BankTransactionAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'number', 'bank_account', 'transaction_date']

@admin.register(TaxReturn)
class TaxReturnAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'number', 'period_start', 'period_end']

from apps.projects.models import ProjectMember, Milestone, TaskComment, TimeLog
...
class MilestoneAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'project', 'name', 'description']
...
from apps.assets.models import AssetCategory, Asset, AssetMaintenance, DepreciationEntry

@admin.register(AssetCategory)
class AssetCategoryAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'name', 'depreciation_method', 'useful_life_years']
...
class DepreciationEntryAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'asset', 'period_start', 'period_end']

from apps.helpdesk.models import TicketReply, KnowledgeBaseArticle

@admin.register(TicketReply)
class TicketReplyAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'ticket', 'author', 'content']
...
class DocumentCategoryAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'name', 'parent', 'description']
...
class DocumentVersionAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'document', 'version', 'file']

from apps.workflow.models import WorkflowDefinition, WorkflowStep, WorkflowInstance, WorkflowAction

@admin.register(WorkflowDefinition)
class WorkflowDefinitionAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'name', 'description', 'trigger_model']

@admin.register(WorkflowStep)
class WorkflowStepAdmin(admin.ModelAdmin):
    list_display = ['workflow', 'name', 'step_order', 'step_type', 'approver_type', 'approver_user']

@admin.register(WorkflowInstance)
class WorkflowInstanceAdmin(admin.ModelAdmin):
    list_display = ['created_by', 'updated_by', 'company', 'definition', 'current_step', 'status']
