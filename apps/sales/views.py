        ...
        if quot.status == Quotation.Status.DRAFT:
            ...
            if quot.customer.email:
                ...
                send_email_task.delay(
                    ...
                    template="quotation",
                    context={
                        "quotation_number": quot.number,
                        "total": float(quot.total),
                    },
                    company_id=str(quot.company_id),
                )
            messages.success(
                request, f"Quotation {quot.number} sent to {quot.customer.name}."
            )
...
class QuotationApproveView(CompanyScopedMixin, View):
    ...
    def post(self, request, pk):
        ...
        if quot.status in (Quotation.Status.CONVERTED, Quotation.Status.REJECTED):
            messages.error(
                request, "Converted or rejected quotations cannot be approved."
            )
            return redirect("sales:quotation_detail", pk=pk)
...
class QuotationConvertToSOView(CompanyScopedMixin, View):
    ...
    def post(self, request, pk):
        ...
        if quot.status not in (Quotation.Status.SENT, Quotation.Status.APPROVED):
            messages.error(
                request, "Quotation must be sent or approved before converting."
            )
            return redirect("sales:quotation_detail", pk=pk)
        ...
        try:
            ...
            if not service.verify_credit_limit(quot.customer, quot.total):
                messages.warning(
                    request,
                    f"Credit limit exceeded for {quot.customer.name}. Order requires approval.",
                )
                ...
                messages.success(
                    request,
                    f"Sales Order {so.number} created (Draft pending approval).",
                )
            ...
            messages.success(
                request, f"Successfully converted to Sales Order {so.number}."
            )
            return redirect("sales:order_detail", pk=so.pk)
        except Exception as e:
...
class SalesOrderCreateView(CompanyScopedMixin, View):
    ...
    def post(self, request):
        ...
        try:
            ...
            messages.success(
                request, f"Sales Order {order.number} created successfully."
            )
...
class InvoiceListView(CompanyScopedMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["total_outstanding"] = (
            qs.filter(status__in=["sent", "partial"]).aggregate(t=Sum("balance_due"))[
                "t"
            ]
            or 0
        )
        ctx["total_overdue"] = (
            qs.filter(
                status__in=["sent", "partial"], due_date__lt=timezone.now().date()
...
class InvoiceUpdateView(CompanyScopedMixin, View):
    ...
    def get(self, request, pk):
        ...
        if invoice.sales_order:
            messages.error(
                request,
                "Invoices generated from Sales Orders cannot be edited directly.",
            )
...
class InvoiceDeleteView(CompanyScopedMixin, View):
    ...
    def post(self, request, pk):
        ...
        if invoice.sales_order:
            messages.error(
                request,
                "Invoices generated from Sales Orders cannot be deleted directly.",
            )
            return redirect("sales:invoices")
        invoice.delete()
...
class InvoiceDetailView(CompanyScopedMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx["lines"] = self.object.lines.all().select_related("product", "tax")
        ctx["payments"] = self.object.payments.filter(status="completed").order_by(
            "payment_date"
        )
        return ctx
...
class InvoiceGeneratePaymentLinkView(CompanyScopedMixin, View):
    ...
    def post(self, request, pk):
        ...
        if invoice.status in ["paid", "void"]:
            messages.error(
                request, "Cannot generate payment link for paid or void invoices."
            )
...
class RecordPaymentView(CompanyScopedMixin, View):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            messages.success(
                request,
                f"Payment of {payment.amount} recorded for invoice {invoice.number}.",
            )
        except ValueError as e:
            messages.error(request, str(e))
        except Exception as e:
...
class PaymentListView(CompanyScopedMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["total_received"] = (
            base_qs.filter(status="completed").aggregate(t=Sum("amount"))["t"] or 0
        )
        ctx["total_pending"] = (
            base_qs.filter(status="pending").aggregate(t=Sum("amount"))["t"] or 0
        )
...
urlpatterns = [
    ...
    path(
        "quotations/<uuid:pk>/", QuotationDetailView.as_view(), name="quotation_detail"
    ),
    path(
        "quotations/<uuid:pk>/send/", QuotationSendView.as_view(), name="quotation_send"
    ),
    path(
        "quotations/<uuid:pk>/convert/",
        QuotationConvertToSOView.as_view(),
    ...
    path(
        "orders/<uuid:pk>/invoice/",
        CreateInvoiceFromSOView.as_view(),
        name="order_invoice",
    ),
    # Invoices
...
class SalesCommissionListView(CompanyScopedMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if (
            ...
            and self.request.user.role != "company_admin"
        ):
...
class SalesCommissionPayView(CompanyScopedMixin, View):
    ...
    def post(self, request, pk):
        if not (
            ...
            or request.user.role in ["company_admin", "super_admin"]
        ):
        ...
        if commission.status == SalesCommission.Status.PENDING:
            ...
            messages.success(
                request,
                f"Commission for {commission.sales_rep.full_name} marked as paid.",
            )
        else:
...
class SubscriptionGenerateInvoiceView(CompanyScopedMixin, View):
    ...
    def post(self, request, pk):
        ...
        if sub.status != Subscription.Status.ACTIVE:
            messages.error(
                request, "Cannot generate invoice for inactive subscription."
            )
        ...
        if sub.billing_cycle == "monthly":
            ...
            sub.next_billing_date = sub.next_billing_date.replace(
                year=sub.next_billing_date.year + 1
            )
        ...
        messages.success(
            request,
            f"Invoice {inv.number} generated successfully. Next billing date advanced.",
        )
        return redirect("sales:invoice_detail", pk=inv.pk)
...
class SalesDashboardView(CompanyScopedMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Summary Metrics
        total_rev = (
            Invoice.objects.filter(company=c, status__in=["paid", "partial"]).aggregate(
                t=Sum("amount_paid")
            )["t"]
            or 0
        )
        ctx["total_revenue"] = total_rev
        ctx["outstanding_receivables"] = (
            Invoice.objects.filter(
                company=c, status__in=["sent", "partial", "overdue"]
            ).aggregate(t=Sum("balance_due"))["t"]
            or 0
        )
        ctx["orders_count"] = SalesOrder.objects.filter(
        ...
        # Recent Orders
        ctx["recent_orders"] = (
            SalesOrder.objects.filter(company=c)
        ...
        try:
            ...
            if not currency:
                return JsonResponse(
                    {
                        "success": False,
                        "error": "No currency configured. Please add a currency in company settings.",
                    },
                    status=400,
                )
