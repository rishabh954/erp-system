...
class DocumentListView(CompanyMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['categories']    = DocumentCategory.objects.filter(company=self.company(), is_deleted=False)
...
class DocumentDetailView(CompanyMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['versions'] = self.object.versions.filter(
            is_deleted=False
...
class DocumentUploadView(CompanyMixin, View):
    ...
    def get(self, request):
        return render(request, self.template_name, {
            'categories': DocumentCategory.objects.filter(company=self.company(), is_deleted=False),
    ...
    def post(self, request):
        ...
        try:
            ...
            doc.number = BaseService.generate_sequence_number('DOC', Document, company.pk)
            doc.save()
...
class DocumentNewVersionView(CompanyMixin, View):
    def post(self, request, pk):
        doc  = get_object_or_404(Document, pk=pk, company=self.company(), is_deleted=False)
        file = request.FILES.get('file')
...
urlpatterns = [
    ...
    path('categories/create/', DocumentCategoryCreateView.as_view(), name='category_create'),
]
