...
from apps.inventory.models import Product, ProductCategory, Warehouse, StockMovement

from apps.pos.models import POSSession, POSOrder, POSOrderLine, POSPayment
import json
...
class POSIndexView(CompanyMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        session = POSSession.objects.filter(user=request.user, status=POSSession.Status.OPEN).first()
        if not session:
        ...
        products = Product.objects.filter(company=self.company(), is_active=True).select_related('category')
