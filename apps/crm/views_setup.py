...
from .models import Territory, LeadAssignmentRule, SalesTarget
...
class LeadAssignmentRuleCreateView(CompanyMixin, CreateView):
    ...
    fields = ['name', 'is_active', 'source_criteria', 'min_revenue', 'assignment_method', 'target_territory', 'priority']
...
class SalesTargetListView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        return SalesTarget.objects.filter(company=self.company()).order_by('-start_date')

class SalesTargetCreateView(CompanyMixin, CreateView):
    model = SalesTarget
    fields = ['sales_rep', 'period', 'start_date', 'end_date', 'target_revenue', 'target_deals']
