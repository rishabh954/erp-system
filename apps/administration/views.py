...
from .models import (
    Designation, NumberSeries, ApprovalMatrix,
    EmailConfig, SMSConfig, WhatsAppConfig, NotificationTemplate,
...
class AdminDashboardView(AdminRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            'total_users': company.users.filter(is_active=True).count() if company else 0,
            'total_api_keys': APIKey.objects.filter(company=company, status='active').count() if company else 0,
            'recent_audit_logs': AuditLog.objects.filter(company=company).order_by('-timestamp')[:10] if company else [],
            'recent_activity': ActivityLog.objects.filter(company=company).order_by('-timestamp')[:10] if company else [],
        })
        return ctx
...
class DesignationListView(HRManagerOrAdminMixin, View):
    ...
    def get(self, request):
        designations = Designation.objects.filter(company=self.company).order_by('level', 'name')
        return render(request, self.template_name, {'designations': designations, 'company': self.company})
...
class NumberSeriesListView(AdminRequiredMixin, View):
    ...
    def get(self, request):
        ...
        return render(request, self.template_name, {'series': series, 'doc_types': doc_types})

    def post(self, request):
        dt = request.POST.get('doc_type')
        NumberSeries.objects.update_or_create(
            company=self.company, doc_type=dt,
            defaults={
...
class SMSConfigView(AdminRequiredMixin, View):
    ...
    def get(self, request):
        ...
        return render(request, self.template_name, {
            'configs': configs, 'providers': SMSConfig.Provider.choices,
        })
...
class WhatsAppConfigView(AdminRequiredMixin, View):
    ...
    def get(self, request):
        ...
        return render(request, self.template_name, {
            'configs': configs, 'providers': WhatsAppConfig.Provider.choices,
        })
...
class DataImportView(AdminRequiredMixin, View):
    ...
    def get(self, request):
        jobs = ImportJob.objects.filter(company=self.company).order_by('-created_at')[:20]
        exports = ExportJob.objects.filter(company=self.company).order_by('-created_at')[:20]
        return render(request, self.template_name, {
            'import_jobs': jobs, 'export_jobs': exports,
    ...
    def post(self, request):
        ...
        if action == 'import' and request.FILES.get('file'):
            ...
            messages.success(request, 'Import job submitted. Processing will begin shortly.')
        elif action == 'export':
            ...
            messages.success(request, 'Export job queued. You will be notified when ready.')
...
class BackupRestoreView(AdminRequiredMixin, View):
    ...
    def get(self, request):
        backups = BackupRecord.objects.filter(company=self.company).order_by('-created_at')[:20]
        return render(request, self.template_name, {
            'backups': backups, 'backup_types': BackupRecord.BackupType.choices,
        })
    ...
    def post(self, request):
        ...
        if action == 'create':
            ...
            messages.success(request, 'Backup job initiated. You will be notified when complete.')
...
class IntegrationCenterView(AdminRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if action == 'create_integration':
            ...
            Integration.objects.filter(pk=request.POST.get('pk'), company=self.company).delete()
            messages.success(request, 'Integration removed.')
        elif action == 'delete_webhook':
            WebhookEndpoint.objects.filter(pk=request.POST.get('pk'), company=self.company).delete()
...
class DashboardBuilderView(AdminRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if action == 'create_dashboard':
            ...
            CustomDashboard.objects.filter(pk=request.POST.get('pk'), company=self.company).delete()
...
class ReportBuilderView(AdminRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if action == 'create':
            ...
            CustomReport.objects.filter(pk=request.POST.get('pk'), company=self.company).delete()
