...
class Designation(CompanyScoped):
    ...
    department = models.ForeignKey(
        'company.Department', null=True, blank=True,
        on_delete=models.SET_NULL, related_name='designations',
    )
...
class NumberSeries(CompanyScoped):
    ...
    def get_next_number(self):
        ...
        with transaction.atomic():
            ...
            if '{FY}' in prefix or '{FY}' in suffix or '{FY_SHORT}' in prefix or '{FY_SHORT}' in suffix:
                from apps.company.models import FiscalYear
                fy = FiscalYear.objects.filter(company=obj.company, is_current=True).first()
                ...
                prefix = prefix.replace('{FY}', fy_str).replace('{FY_SHORT}', fy_short_str)
                suffix = suffix.replace('{FY}', fy_str).replace('{FY_SHORT}', fy_short_str)
...
class ApprovalMatrix(CompanyScoped):
    ...
    approver_user = models.ForeignKey(
        ...
        on_delete=models.SET_NULL, related_name='approval_assignments',
    )
...
class SMSConfig(CompanyScoped):
    ...
    provider = models.CharField(max_length=20, choices=Provider.choices, default=Provider.TWILIO)
...
class WhatsAppConfig(CompanyScoped):
    ...
    provider = models.CharField(max_length=20, choices=Provider.choices, default=Provider.META_CLOUD)
...
class NotificationTemplate(CompanyScoped):
    ...
    trigger_event = models.CharField(max_length=50, choices=TriggerEvent.choices, default=TriggerEvent.CUSTOM)
    subject = models.CharField(max_length=500, blank=True, help_text='Email subject (supports {{variables}})')
...
class ExportJob(CompanyScoped):
    ...
    format = models.CharField(max_length=10, choices=[('csv', 'CSV'), ('xlsx', 'Excel'), ('pdf', 'PDF')], default='csv')
    status = models.CharField(max_length=15, choices=Status.choices, default=Status.PENDING)
...
class BackupRecord(CompanyScoped):
    ...
    initiated_by = models.ForeignKey(
        'authentication.User', on_delete=models.SET_NULL, null=True, related_name='backups',
    )
...
class AuditLog(UUIDModel):
    ...
    company = models.ForeignKey('company.Company', null=True, blank=True, on_delete=models.SET_NULL, related_name='admin_audit_logs')
    user = models.ForeignKey('authentication.User', null=True, blank=True, on_delete=models.SET_NULL, related_name='admin_audit_logs')
...
class ActivityLog(UUIDModel):
    ...
    company = models.ForeignKey('company.Company', null=True, blank=True, on_delete=models.SET_NULL, related_name='admin_activity_logs')
    user = models.ForeignKey('authentication.User', null=True, blank=True, on_delete=models.SET_NULL, related_name='admin_activity_logs')
...
class APIKey(CompanyScoped):
    ...
    created_by = models.ForeignKey(
        'authentication.User', on_delete=models.SET_NULL, null=True, related_name='api_keys',
    )
...
class Integration(CompanyScoped):
    ...
    provider = models.CharField(max_length=100, help_text='e.g. Razorpay, Shopify, Shiprocket')
...
class CustomDashboard(CompanyScoped):
    ...
    owner = models.ForeignKey(
        'authentication.User', on_delete=models.CASCADE, related_name='dashboards',
    )
...
class DashboardWidget(CompanyScoped):
    ...
    data_source = models.CharField(max_length=200, help_text='API endpoint or query name')
...
class CustomReport(CompanyScoped):
    ...
    base_model = models.CharField(max_length=100, help_text='e.g. apps.sales.models.Invoice')
    ...
    report_format = models.CharField(max_length=10, choices=ReportFormat.choices, default=ReportFormat.TABLE)
    chart_config = models.JSONField(default=dict)
    is_public = models.BooleanField(default=False)
    created_by = models.ForeignKey(
        'authentication.User', on_delete=models.SET_NULL, null=True, related_name='custom_reports',
    )
