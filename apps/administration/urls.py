...
from . import integration_views, security_views, views
...
urlpatterns = [
    ...
    path(
        "approval-matrix/",
        views.ApprovalMatrixListView.as_view(),
        name="approval_matrix",
    ),
    path(
        "approval-matrix/<uuid:pk>/delete/",
        views.ApprovalMatrixDeleteView.as_view(),
        name="approval_matrix_delete",
    ),
    # Users
    path(
        "pending-approvals/",
        views.PendingUserApprovalListView.as_view(),
        name="pending_approvals",
    ),
    path(
        "pending-approvals/<uuid:pk>/action/",
        views.PendingUserApprovalActionView.as_view(),
        name="pending_approvals_action",
    ),
    # Communication
    path("email-config/", views.EmailConfigView.as_view(), name="email_config"),
    path(
        "email-config/<uuid:pk>/delete/",
        views.EmailConfigDeleteView.as_view(),
        name="email_config_delete",
    ),
    path(
        "email-config/<uuid:pk>/test/",
        views.EmailConfigTestView.as_view(),
        name="email_config_test",
    ),
    path("sms-config/", views.SMSConfigView.as_view(), name="sms_config"),
    path(
        "whatsapp-config/", views.WhatsAppConfigView.as_view(), name="whatsapp_config"
    ),
    path(
        "notifications/", views.NotificationCenterView.as_view(), name="notifications"
    ),
    # Documents
    path(
        "document-templates/",
    ...
    path(
        "integrations/",
        integration_views.IntegrationsDashboardView.as_view(),
        name="integrations_dashboard",
    ),
    path(
        "integrations/setup/<str:provider>/",
        integration_views.GenericIntegrationSetupView.as_view(),
        name="integration_setup",
    ),
    path(
        "integrations/oauth/<str:provider>/",
        integration_views.OAuthMockConnectView.as_view(),
        name="oauth_connect",
    ),
    path(
        "integrations/webhooks/",
        integration_views.WebhookManagementView.as_view(),
        name="webhooks",
    ),
    path(
        "integrations/import/",
        integration_views.DataImportView.as_view(),
        name="data_import",
    ),
    ...
    path(
        "api-keys/<uuid:pk>/revoke/",
        security_views.APIKeyRevokeView.as_view(),
        name="api_key_revoke",
    ),
    path(
        "backup-schedule/",
        security_views.BackupSchedulerView.as_view(),
    ...
    path(
        "dashboard-builder/",
        views.DashboardBuilderView.as_view(),
        name="dashboard_builder",
    ),
    path("report-builder/", views.ReportBuilderView.as_view(), name="report_builder"),
    # System
    path(
        "system-settings/", views.SystemSettingsView.as_view(), name="system_settings"
    ),
]
