...
from .models import Project, Sprint, TimeLog
...
class ProjectTrackingService:
    ...
    def get_sprint_burndown(sprint_id):
        ...
        total_estimated = tasks.aggregate(total=Sum("estimated_hours"))[
            "total"
        ] or Decimal(0)
        ...
        for i in range(days + 1):
            ...
            ideal_data.append(
                {"date": date.strftime("%Y-%m-%d"), "remaining": float(ideal_remaining)}
            )
        ...
        total_actual_hours = tasks.aggregate(total=Sum("actual_hours"))[
            "total"
        ] or Decimal(0)
