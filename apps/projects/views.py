...
from .models import Project, Task, Milestone, TaskComment, TimeLog, ProjectMember
...
class ProjectListView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        ...
        ).select_related('manager', 'customer').annotate(
            task_count=Count('tasks', filter=Q(tasks__is_deleted=False)),
            done_count=Count('tasks', filter=Q(tasks__status='done', tasks__is_deleted=False)),
        ).order_by('-created_at')
        ...
        if self.request.user.role not in ('project_manager', 'company_admin', 'super_admin'):
            qs = qs.filter(
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.GET.get('open_kanban'):
            first_proj = Project.objects.filter(
                company=self.company(), is_deleted=False, status__in=['active', 'planning', 'on_hold']
...
class ProjectDetailView(CompanyMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        project = self.object
        ctx['milestones'] = project.milestones.filter(is_deleted=False).order_by('due_date')
        ctx['tasks'] = project.tasks.filter(is_deleted=False).select_related(
            'assigned_to', 'milestone'
        ).order_by('status', 'position')
        ctx['members'] = ProjectMember.objects.filter(project=project).select_related('user')
        ctx['time_logs'] = TimeLog.objects.filter(
            task__project=project, is_deleted=False
        ).select_related('user', 'task').order_by('-date')[:20]
        ctx['total_hours'] = TimeLog.objects.filter(
            task__project=project, is_deleted=False
        ).aggregate(t=Sum('hours'))['t'] or 0
        ctx['budget_used_pct'] = int(
...
class ProjectCreateView(CompanyMixin, View):
    ...
    def get(self, request):
        ...
        return render(request, self.template_name, {
            'customers': Customer.objects.filter(company=c, is_deleted=False).order_by('name'),
            'users': User.objects.filter(companies=c, is_active=True).order_by('first_name'),
    ...
    def post(self, request):
        ...
        try:
            ...
            project.number = BaseService.generate_sequence_number('PRJ', Project, company.pk)
            project.save()
    ...
    def get_context_data(self, **kwargs):
        ...
        for s in statuses:
            columns[s] = {
                ...
                ).select_related('assigned_to').order_by('position'),
            }
        ctx['columns'] = columns
...
class TaskCreateView(CompanyMixin, View):
    def get(self, request):
        ...
        TaskForm = modelform_factory(
            Task, 
            fields=['project', 'title', 'description', 'status', 'priority', 'assigned_to', 'due_date', 'estimated_hours'],
            widgets={'due_date': DateInput(attrs={'type': 'date'})}
        )
        form = TaskForm()
        form.fields['project'].queryset = Project.objects.filter(company=self.company(), is_deleted=False)
        from apps.authentication.models import User
        form.fields['assigned_to'].queryset = User.objects.filter(projectmember__project__company=self.company(), is_active=True).distinct()
...
class MyTasksView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        ...
        ).select_related('project', 'milestone').order_by('due_date', 'priority')

    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['total_hours_logged'] = TimeLog.objects.filter(
...
class TaskDetailView(CompanyMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['comments'] = self.object.comments.filter(is_deleted=False).select_related('author')
        ctx['time_logs'] = self.object.time_logs.filter(is_deleted=False).select_related('user')
        ctx['subtasks'] = self.object.subtasks.filter(is_deleted=False)
        ctx['total_logged'] = self.object.time_logs.filter(is_deleted=False).aggregate(
            t=Sum('hours')
        )['t'] or 0
        return ctx
    ...
    def post(self, request, pk):
        ...
        if content:
            TaskComment.objects.create(
                company=self.company(), task=task,
