...
from apps.inventory.services import StockService, TransferService, InventoryAnalyticsService
from apps.inventory.models import StockRecord, StockMovement, InventoryTransfer, Product
...
class TestMovements:
    ...
    def test_adjust_stock_set(self, company, user, product, warehouse):
        ...
        service.receive_stock(
            product=product, warehouse=warehouse, qty=Decimal('50.00'), 
            unit_cost=Decimal('10.00'), reference_type='PO', reference_id='1'
        )
        
        mov = service.adjust_stock(
            product=product, warehouse=warehouse, qty_input=Decimal('30.00'), 
            adjustment_type='set', notes='Found less'
        )
    ...
    def test_adjust_stock_add(self, company, user, product, warehouse):
        service = StockService(company=company, user=user)
        mov = service.adjust_stock(
            product=product, warehouse=warehouse, qty_input=Decimal('10.00'), 
            adjustment_type='add', notes='Found more'
        )
    ...
    def test_adjust_stock_remove(self, company, user, product, warehouse):
        ...
        mov = service.adjust_stock(
            product=product, warehouse=warehouse, qty_input=Decimal('10.00'), 
            adjustment_type='remove', notes='Lost'
        )
    ...
    def test_reserve_and_release_stock(self, company, user, product, warehouse):
        ...
        stock = service.reserve_stock(
            product=product, warehouse=warehouse, qty=Decimal('20.00'), 
            reference_type='SO', reference_id='1'
        )
        ...
        stock = service.release_reservation(
            product=product, warehouse=warehouse, qty=Decimal('10.00'), 
            reference_type='SO', reference_id='1'
        )
    ...
    def test_transfer_workflow(self, company, user, product, warehouse, to_warehouse):
        ...
        stock_service.receive_stock(
            product=product, warehouse=warehouse, qty=Decimal('100.00'), 
            unit_cost=Decimal('10.00'), reference_type='PO', reference_id='1'
        )
        ...
        # 3. Ship Transfer
        ship_data = {
            f'qty_sent_{transfer.lines.first().id}': '40'
        }
        ...
        # 4. Receive Transfer
        receive_data = {
            f'qty_recv_{transfer.lines.first().id}': '40'
        }
        transfer = transfer_service.process_transfer(transfer, 'receive', receive_data, user)
    ...
    def test_transfer_no_lines(self, company, user, warehouse, to_warehouse):
        ...
        with pytest.raises(ValueError, match='must have at least one valid product line'):
            transfer_service.create_transfer(data, user)
