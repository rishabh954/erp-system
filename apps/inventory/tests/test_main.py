...
def inventory_services(db, rf, company, user, warehouse, product):
    from core.factories import VendorFactory, CustomerFactory
    vendor = VendorFactory(company=company, name='Test Vendor')
...
def test_reservation_blocks_oversell(inventory_services):
    ...
    for w in [wh, inventory_services['warehouse2']]:
        StockRecord.objects.create(company=company, product=product, warehouse=w, quantity_on_hand=Decimal('10'), quantity_reserved=0)
    
    # Order 1 wants 8
    so1 = SalesOrder.objects.create(company=company, customer=inventory_services['customer'], order_date='2026-01-01', number='SO-1')
    SalesOrderLine.objects.create(sales_order=so1, product=product, quantity=Decimal('8'), unit_price=Decimal('10'))
    ...
    # Order 2 wants 5
    so2 = SalesOrder.objects.create(company=company, customer=inventory_services['customer'], order_date='2026-01-02', number='SO-2')
    SalesOrderLine.objects.create(sales_order=so2, product=product, quantity=Decimal('5'), unit_price=Decimal('10'))
...
def test_reservation_released_on_shipment(inventory_services):
    ...
    so = SalesOrder.objects.create(company=company, customer=inventory_services['customer'], order_date='2026-01-01', number='SO-3')
    SalesOrderLine.objects.create(sales_order=so, product=product, quantity=Decimal('4'), unit_price=Decimal('10'))
    ...
    do = DeliveryOrder.objects.create(company=company, sales_order=so, warehouse=active_wh, status=DeliveryOrder.Status.READY)
    DeliveryOrderLine.objects.create(delivery_order=do, product=product, quantity_ordered=Decimal('4'), quantity_shipped=Decimal('4'))
...
def test_reservation_released_on_cancellation(inventory_services):
    ...
    so = SalesOrder.objects.create(company=company, customer=inventory_services['customer'], order_date='2026-01-01', number='SO-4')
    SalesOrderLine.objects.create(sales_order=so, product=product, quantity=Decimal('4'), unit_price=Decimal('10'))
...
def test_lot_tracked_product_requires_batch(inventory_services):
    from apps.inventory.models import Product, DeliveryOrder, DeliveryOrderLine, StockRecord
    ...
    StockRecord.objects.filter(company=company, product=product, warehouse=wh).delete()
    StockRecord.objects.create(company=company, product=product, warehouse=wh, quantity_on_hand=Decimal('10'), quantity_reserved=0)
    
    so = SalesOrder.objects.create(company=company, customer=inventory_services['customer'], order_date='2026-01-01', number='SO-5')
    do = DeliveryOrder.objects.create(company=company, sales_order=so, warehouse=wh, status=DeliveryOrder.Status.READY)
    DeliveryOrderLine.objects.create(delivery_order=do, product=product, quantity_ordered=Decimal('4'), quantity_shipped=Decimal('4'))
...
def test_serial_tracked_product_requires_matching_serial_count(inventory_services):
    ...
    so = SalesOrder.objects.create(company=company, customer=inventory_services['customer'], order_date='2026-01-01', number='SO-6')
    do = DeliveryOrder.objects.create(company=company, sales_order=so, warehouse=wh, status=DeliveryOrder.Status.READY)
    # Ship 4, but only provide 2 serial numbers
    DeliveryOrderLine.objects.create(
        delivery_order=do, product=product, quantity_ordered=Decimal('4'), 
        quantity_shipped=Decimal('4'), serial_numbers=["SN-1", "SN-2"]
    )
...
@pytest.mark.django_db
def test_serial_tracked_product_ships_successfully_with_correct_serials(inventory_services):
    from apps.inventory.models import Product, DeliveryOrder, DeliveryOrderLine, StockRecord, StockMovement
    from apps.sales.models import SalesOrder
    company = inventory_services['company']
    user = inventory_services['user']
    wh = inventory_services['warehouse']
    ...
    so = SalesOrder.objects.create(company=company, customer=inventory_services['customer'], order_date='2026-01-01', number='SO-7')
    do = DeliveryOrder.objects.create(company=company, sales_order=so, warehouse=wh, status=DeliveryOrder.Status.READY)
    # Ship 2 with 2 serial numbers
    DeliveryOrderLine.objects.create(
        delivery_order=do, product=product, quantity_ordered=Decimal('2'), 
        quantity_shipped=Decimal('2'), serial_numbers=["SN-9", "SN-10"]
    )
    ...
    mov = StockMovement.objects.get(reference_type='DeliveryOrder', reference_id=str(do.id))
