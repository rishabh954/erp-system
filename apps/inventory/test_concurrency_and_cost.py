...
def test_receive_stock_weighted_average_cost(company, user):
    ...
    product = ProductFactory(company=company, name='Average Cost Widget', sku='WGT-AVG', cost_price=Decimal('10.00'))
...
def test_adjust_stock_negative_guard(company, user):
    ...
    product = ProductFactory(company=company, name='Guard Widget', sku='WGT-GRD', cost_price=Decimal('10.00'))
    warehouse = WarehouseFactory(company=company, name='Main Warehouse', code='MAIN')
    service = StockService(user=user, company=company)
    
    # Receive 5 units
    service.receive_stock(
        product=product, warehouse=warehouse, qty=Decimal('5.00'), unit_cost=Decimal('10.00'), reference_type='GRN', reference_id='1'
    )
