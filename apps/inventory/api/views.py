...
from apps.inventory.api.serializers import (
    ...
    StockMovementSerializer, InventoryTransferSerializer
)
...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def adjust_stock(self, request, pk=None):
        ...
        if not warehouse_id or quantity is None or adjustment_type not in ['add', 'remove', 'set']:
        ...
        return Response({'status': 'stock_adjusted', 'product': product.name, 'warehouse': warehouse.name})
...
class InventoryTransferViewSet(viewsets.ModelViewSet):
    ...
    def approve(self, request, pk=None):
        ...
        if transfer.status != InventoryTransfer.Status.DRAFT:
            return Response({'error': 'Can only approve draft transfers'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def receive(self, request, pk=None):
        transfer = self.get_object()
        if transfer.status not in [InventoryTransfer.Status.APPROVED, InventoryTransfer.Status.IN_TRANSIT]:
            return Response({'error': 'Can only receive approved/in-transit transfers'}, status=status.HTTP_400_BAD_REQUEST)
