...
class BarcodeScanAPIView(LoginRequiredMixin, View):
    def post(self, request):
        try:
            ...
            if not barcode:
                return JsonResponse(
                    {"success": False, "error": "Barcode is required"}, status=400
                )
            ...
            product = (
                ...
                .filter(models.Q(barcode=barcode) | models.Q(sku=barcode))
                .first()
            )
            ...
            stock_record = (
                StockRecord.objects.filter(product__company=company)
                .filter(
                ...
                return JsonResponse(
                    {
                        ...
                        "data": {
                            ...
                            "bin_location": (
                                stock_record.bin_location.name
                                if stock_record.bin_location
                                else None
                            ),
            ...
            return JsonResponse(
                {"success": False, "error": "Barcode not found"}, status=404
            )
