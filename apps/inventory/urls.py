...
from .views import (
    ...
    ProductDetailView,
    ProductListView,
    ReorderRuleDetailView,
    ReorderRuleListView,
    SerialNumberDetailView,
...
urlpatterns = [
    ...
    path(
        ...
        ProductCategoryAjaxCreateView.as_view(),
    ...
    path(
        "warehouses/<uuid:pk>/update/",
        WarehouseUpdateView.as_view(),
        name="warehouse_update",
    ),
    path(
        "warehouses/<uuid:pk>/delete/",
    ...
    path(
        "deliveries/<uuid:pk>/",
        DeliveryOrderDetailView.as_view(),
        name="delivery_detail",
    ),
    path(
        "deliveries/<uuid:pk>/ship/", ShipDeliveryView.as_view(), name="delivery_ship"
    ),
    # WMS Phase 7
    path("wms/picklists/", PickListListView.as_view(), name="picklists"),
    path(
        "wms/picklists/<uuid:pk>/", PickListDetailView.as_view(), name="picklist_detail"
    ),
    ...
    path(
        "wms/serials/<uuid:pk>/", SerialNumberDetailView.as_view(), name="serial_detail"
    ),
    ...
    path(
        "transfers/<uuid:pk>/action/",
        TransferActionView.as_view(),
        name="transfer_action",
    ),
    # Enterprise Inventory
    path("reorder-rules/", ReorderRuleListView.as_view(), name="reorder_rules"),
    path(
        "reorder-rules/<uuid:pk>/",
        ReorderRuleDetailView.as_view(),
        name="reorder_rule_detail",
    ),
]
