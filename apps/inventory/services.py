...
class InventoryAnalyticsService:
    ...
    def compute_abc_analysis(company):
        ...
        # Aggregate the absolute value of quantity * unit_cost for all active stockable products
        movements_aggregation = (
            StockMovement.objects.filter(
                company=company,
                product__is_active=True,
                product__product_type=Product.ProductType.STOCKABLE,
            ...
            .annotate(
                usage_value=Sum(
                    ExpressionWrapper(
                        F("quantity") * F("unit_cost") * -1, output_field=DecimalField(max_digits=20, decimal_places=4)
                    )
                )
            )
        )
        ...
        usage_map = {
            item["product_id"]: item["usage_value"]
            for item in movements_aggregation
            if item["usage_value"] and item["usage_value"] > 0
        }

        # Fetch all active stockable products in one query
        products = list(
        ...
        for product in products:
            usage = usage_map.get(product.id, Decimal("0"))
            if usage > 0:
        ...
        if products_to_update:
            Product.objects.bulk_update(products_to_update, ["abc_classification"])
        ...
        for item in products_usage:
            ...
            if prod.abc_classification != classification:
                ...
                classified_updates.append(prod)

        if classified_updates:
            Product.objects.bulk_update(classified_updates, ["abc_classification"])
