...
class Product(CompanyScoped, NotesMixin):
    ...
    product_type = models.CharField(
        max_length=15, choices=ProductType.choices, default=ProductType.STOCKABLE
    )
    ...
    tracking_method = models.CharField(
        max_length=10, choices=TrackingMethod.choices, default=TrackingMethod.NONE
    )
...
class StockRecord(CompanyScoped):
    ...
    bin_location = models.ForeignKey(
        BinLocation, null=True, blank=True, on_delete=models.SET_NULL
    )
...
class LotBatch(CompanyScoped, SequenceMixin, NotesMixin):
    ...
    product = models.ForeignKey(
        "inventory.Product", on_delete=models.CASCADE, related_name="lots"
    )
    variant = models.ForeignKey(
        "inventory.ProductVariant", null=True, blank=True, on_delete=models.CASCADE
    )
    ...
    vendor = models.ForeignKey(
        "purchase.Vendor", null=True, blank=True, on_delete=models.SET_NULL
    )
...
class SerialNumber(CompanyScoped, SequenceMixin, NotesMixin):
    ...
    product = models.ForeignKey(
        "inventory.Product", on_delete=models.CASCADE, related_name="serial_numbers"
    )
    variant = models.ForeignKey(
        "inventory.ProductVariant", null=True, blank=True, on_delete=models.CASCADE
    )
...
class PickListLine(models.Model):
    ...
    bin_location = models.ForeignKey(
        "inventory.BinLocation", null=True, blank=True, on_delete=models.SET_NULL
    )
...
class PackingSlip(CompanyScoped, SequenceMixin, NotesMixin):
    ...
    delivery_order = models.ForeignKey(
        "inventory.DeliveryOrder", null=True, blank=True, on_delete=models.SET_NULL
    )
    status = models.CharField(
        max_length=20, choices=Status.choices, default=Status.DRAFT
    )
...
class PackingSlipLine(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=_uuid.uuid4, editable=False)
    packing_slip = models.ForeignKey(
        PackingSlip, on_delete=models.CASCADE, related_name="lines"
    )
    product = models.ForeignKey("inventory.Product", on_delete=models.PROTECT)
...
class Shipment(CompanyScoped, SequenceMixin, NotesMixin):
    ...
    packing_slip = models.ForeignKey(
        PackingSlip, null=True, blank=True, on_delete=models.SET_NULL
    )
    status = models.CharField(
...
class LandedCost(CompanyScoped, SequenceMixin, NotesMixin):
    ...
    receipt = models.ForeignKey(
        "purchase.GoodsReceipt", on_delete=models.CASCADE, related_name="landed_costs"
    )
    status = models.CharField(
        max_length=20, choices=Status.choices, default=Status.DRAFT
    )
    allocation_method = models.CharField(
        max_length=20, choices=AllocationMethod.choices, default=AllocationMethod.VALUE
    )
...
class LandedCostAllocation(models.Model):
    ...
    landed_cost = models.ForeignKey(
        LandedCost, on_delete=models.CASCADE, related_name="allocations"
    )
    product = models.ForeignKey("inventory.Product", on_delete=models.PROTECT)
    cost_category = models.CharField(
        max_length=100, help_text="e.g. Freight, Customs, Insurance"
    )
...
class CycleCount(CompanyScoped, SequenceMixin, NotesMixin):
    ...
    warehouse = models.ForeignKey(
        "inventory.Warehouse", on_delete=models.CASCADE, related_name="cycle_counts"
    )
    status = models.CharField(
        max_length=20, choices=Status.choices, default=Status.DRAFT
    )
