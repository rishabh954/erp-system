...
class BOMListView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        return BillOfMaterial.objects.filter(company=self.company()).select_related('product', 'routing').order_by('-created_at')
...
class BOMCreateView(CompanyMixin, View):
    def get(self, request):
        ...
        return render(request, 'manufacturing/boms/form.html', {'products': products, 'routings': routings})
    ...
    def post(self, request):
        ...
        try:
            ...
            routing = Routing.objects.filter(pk=routing_id, company=self.company()).first() if routing_id else None
...
class MOListView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        qs = ManufacturingOrder.objects.filter(company=self.company()).select_related('product', 'bom').order_by('-created_at')
...
class MOCreateView(CompanyMixin, View):
    def get(self, request):
        ...
        boms = BillOfMaterial.objects.filter(company=self.company(), is_active=True).select_related('product')
        warehouses = Warehouse.objects.filter(company=self.company(), is_active=True, is_deleted=False)
        return render(request, 'manufacturing/orders/form.html', {'boms': boms, 'warehouses': warehouses})
...
from .models import ScrapOrder, DowntimeLog
...
class ScrapOrderListView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        return ScrapOrder.objects.filter(company=self.company()).select_related('product', 'work_center', 'manufacturing_order')
...
class DowntimeLogListView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        return DowntimeLog.objects.filter(company=self.company()).select_related('work_center')
...
class WorkOrderListView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        return WorkOrder.objects.filter(company=self.company()).select_related('manufacturing_order', 'work_center')
...
from .models import QualityCheck, ProductionCosting
...
class QualityCheckListView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        return QualityCheck.objects.filter(company=self.company()).select_related('manufacturing_order', 'work_order', 'inspector')
