...
class WorkCenter(CompanyScoped, NotesMixin):
    ...
    cost_per_hour = models.DecimalField(max_digits=15, decimal_places=2, default=0, help_text=_('Operating cost per hour for landed cost calculation'))
    capacity = models.PositiveIntegerField(default=1, help_text=_('Number of simultaneous operations'))
...
class BillOfMaterial(CompanyScoped, SequenceMixin, NotesMixin):
    ...
    product = models.ForeignKey('inventory.Product', on_delete=models.CASCADE, related_name='boms', help_text=_('The finished product'))
    routing = models.ForeignKey(Routing, null=True, blank=True, on_delete=models.SET_NULL)
    quantity = models.DecimalField(max_digits=15, decimal_places=4, default=1, help_text=_('Quantity of finished product this BOM produces'))
    is_active = models.BooleanField(default=True)
...
class BillOfMaterialLine(models.Model):
    ...
    component = models.ForeignKey('inventory.Product', on_delete=models.PROTECT, help_text=_('Raw material or subassembly'))
...
class ManufacturingOrder(CompanyScoped, SequenceMixin, NotesMixin):
    ...
    quantity_to_produce = models.DecimalField(max_digits=15, decimal_places=4, default=1)
    ...
    # Traceability links
    warehouse = models.ForeignKey('inventory.Warehouse', null=True, blank=True, on_delete=models.SET_NULL, help_text=_('Where finished goods will be stored and raw materials consumed'))
            ...
            if self.bom.routing:
                for op in self.bom.routing.operations.all():
                    WorkOrder.objects.create(
                        ...
                        name=op.name,
                        expected_duration_minutes=op.duration_minutes * self.quantity_to_produce
                    )
    ...
    def _process_inventory_transfer(self):
        ...
        for line in self.bom.lines.all():
            qty_to_consume = Decimal(line.quantity) * (Decimal(self.quantity_produced) / Decimal(self.bom.quantity))
            if line.scrap_percentage > 0:
                qty_to_consume += qty_to_consume * (Decimal(line.scrap_percentage) / Decimal(100))
...
class WorkOrder(CompanyScoped, SequenceMixin):
    ...
    operation = models.ForeignKey(RoutingOperation, null=True, blank=True, on_delete=models.SET_NULL)
    work_center = models.ForeignKey(WorkCenter, on_delete=models.PROTECT)
    name = models.CharField(max_length=200)
...
class ScrapOrder(CompanyScoped, SequenceMixin, NotesMixin):
    ...
    manufacturing_order = models.ForeignKey(ManufacturingOrder, null=True, blank=True, on_delete=models.SET_NULL, related_name='scrap_orders')
    ...
    def mark_done(self):
        if self.status == self.Status.DRAFT:
            ...
            StockMovement.objects.create(
                company=self.company,
                product=self.product,
                warehouse=self.manufacturing_order.warehouse if self.manufacturing_order else None,
...
class DowntimeLog(CompanyScoped):
    work_center = models.ForeignKey(WorkCenter, on_delete=models.CASCADE, related_name='downtime_logs')
    reason = models.CharField(max_length=255)
    start_time = models.DateTimeField()
    end_time = models.DateTimeField(null=True, blank=True)
    duration_minutes = models.PositiveIntegerField(default=0, help_text=_("Calculated automatically on save if end_time provided"))
...
class QualityCheck(CompanyScoped, SequenceMixin, NotesMixin):
    ...
    work_order = models.ForeignKey('WorkOrder', null=True, blank=True, on_delete=models.SET_NULL, related_name='quality_checks')
    ...
    parameters_checked = models.TextField(blank=True, help_text=_("Describe what was checked and the results"))
...
class ProductionCosting(CompanyScoped):
    manufacturing_order = models.OneToOneField(ManufacturingOrder, on_delete=models.CASCADE, related_name='costing')
    ...
    def calculate_costs(self):
        ...
        self.material_cost = m_cost * mo.quantity_produced if mo.quantity_produced else m_cost * mo.quantity_to_produce
