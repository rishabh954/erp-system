...
class WorkOrderInline(admin.TabularInline):
    model = WorkOrder
    extra = 0
    readonly_fields = ['number', 'work_center', 'name', 'status', 'expected_duration_minutes', 'actual_duration_minutes']
    can_delete = False

@admin.register(ManufacturingOrder)
class ManufacturingOrderAdmin(admin.ModelAdmin):
    list_display = ['number', 'product', 'quantity_to_produce', 'status', 'planned_start_date']
    search_fields = ['number', 'product__name']
