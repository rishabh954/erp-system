...
from apps.inventory.models import Product, StockRecord, StockMovement, Warehouse
from apps.manufacturing.models import BillOfMaterial, BillOfMaterialLine, ManufacturingOrder, MaterialPlan, MaterialPlanItem, ScrapOrder, WorkCenter
...
def test_bom_creation_mo_and_mrp(client, company, user, warehouse):
    ...
    fg = Product.objects.create(
        company=company, name="Finished Good", sku="FG-01", product_type="stockable", cost_price=Decimal("100.00"), sale_price=Decimal("200.00")
    )
    comp1 = Product.objects.create(
        company=company, name="Component 1", sku="COMP-01", product_type="stockable", cost_price=Decimal("10.00"), sale_price=Decimal("20.00")
    )
    comp2 = Product.objects.create(
        company=company, name="Component 2", sku="COMP-02", product_type="stockable", cost_price=Decimal("15.00"), sale_price=Decimal("25.00")
    )
    ...
    response = client.post(bom_create_url, {
        'product': str(fg.id),
        'quantity': '1',
    ...
    response = client.post(mo_create_url, {
        'bom': str(bom.id),
    ...
    movements_out = StockMovement.objects.filter(reference_id=f"MO-{mo.number}", movement_type='production_out')
    ...
    movements_in = StockMovement.objects.filter(reference_id=f"MO-{mo.number}", movement_type='production_in')
...
def test_mo_cancel_and_scrap(client, company, user, warehouse):
    ...
    bom = BillOfMaterial.objects.create(
        company=company, product=fg, quantity=1
    )
    ...
    wc = WorkCenter.objects.create(
        company=company, name="Assembly Line", code="AL-01", cost_per_hour=Decimal('50.00')
    )
    scrap = ScrapOrder.objects.create(
        company=company, manufacturing_order=mo, work_center=wc, product=comp1, quantity=Decimal('1'), reason="Damaged"
    )
