...
from apps.manufacturing.models import WorkCenter, Routing, RoutingOperation, ManufacturingOrder
...
def test_manufacturing_views(client, company, user, warehouse):
    ...
    RoutingOperation.objects.create(routing=r1, work_center=wc1, name="Op1", sequence=10, duration_minutes=60)
...
def test_work_order_actions(client, company, user, warehouse, product):
    ...
    mo = ManufacturingOrder.objects.create(company=company, product=product, bom=bom, quantity_to_produce=1)
    
    wc1 = WorkCenter.objects.create(company=company, name="WC1", code="WC1")
    wo = WorkOrder.objects.create(company=company, manufacturing_order=mo, work_center=wc1, name="WO1")
    
    # List
    assert client.get(reverse('manufacturing:work_orders')).status_code == 200
    
    # Detail
    assert client.get(reverse('manufacturing:work_order_detail', kwargs={'pk': wo.id})).status_code == 200
