...
from apps.purchase.models import (
    ...
    RequestForQuotation, RFQLine, VendorBid, VendorBidLine,
...
class PurchaseRequestService(BaseService):
    ...
    def create_request(self, data, user):
        ...
        pr.number = BaseService.generate_sequence_number('PR', PurchaseRequest, self.company.pk)
        pr.save()
        ...
        for i, desc in enumerate(descs):
            ...
            price = Decimal(str(est_prices[i])) if (i < len(est_prices) and est_prices[i]) else Decimal('0')
...
class PurchaseOrderService(BaseService):
    ...
    def create_bill(self, order):
        ...
        if order.status not in [PurchaseOrder.Status.CONFIRMED, PurchaseOrder.Status.PARTIAL, PurchaseOrder.Status.RECEIVED]:
    ...
    def create_goods_receipt(self, order, data, user):
        from apps.inventory.models import Warehouse, StockRecord, StockMovement
        ...
        # Update PO status
        order.status = PurchaseOrder.Status.RECEIVED if all_received else PurchaseOrder.Status.PARTIAL
...
class VendorBidService(BaseService):
    ...
    def create_bid(self, rfq, data):
        ...
        for i, line in enumerate(rfq.lines.all()):
            price = Decimal(str(prices[i])) if (i < len(prices) and prices[i]) else Decimal('0')
    ...
    def accept_bid(self, bid, user):
        ...
        po = PurchaseOrder.objects.create(
            company=self.company,
            number=BaseService.generate_sequence_number('PO', PurchaseOrder, self.company.pk),
            vendor=bid.vendor,
        ...
        VendorBid.objects.filter(rfq=bid.rfq, status=VendorBid.Status.PENDING).update(status=VendorBid.Status.REJECTED)
