...
from .models import Vendor, RequestForQuotation, VendorBid, PurchaseOrder, Bill
...
class VendorPortalDashboardView(VendorPortalMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['vendor'] = vendor
        ctx['active_rfqs'] = RequestForQuotation.objects.filter(status='published').count()
        ctx['pending_orders'] = PurchaseOrder.objects.filter(vendor=vendor, status__in=['sent', 'confirmed']).count()
        ctx['unpaid_bills'] = Bill.objects.filter(vendor=vendor, status__in=['open', 'partial']).count()
        
        ctx['recent_orders'] = PurchaseOrder.objects.filter(vendor=vendor).order_by('-order_date')[:5]
        return ctx
...
class VendorPortalRFQListView(VendorPortalMixin, ListView):
    ...
    def get_queryset(self):
        return RequestForQuotation.objects.filter(
            company=self.get_vendor().company,
            status='published'
        ).order_by('-created_at')
...
class VendorPortalRFQDetailView(VendorPortalMixin, DetailView):
    ...
    def get_object(self):
        return get_object_or_404(RequestForQuotation, pk=self.kwargs['pk'], company=self.get_vendor().company)
...
class VendorPortalBidCreateView(VendorPortalMixin, View):
    ...
    def get(self, request, rfq_pk):
        rfq = get_object_or_404(RequestForQuotation, pk=rfq_pk, company=self.get_vendor().company)
