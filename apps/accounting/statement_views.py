...
class BankStatementListView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        return BankStatement.objects.filter(bank_account__company=self.company()).select_related('bank_account')

class BankStatementCreateView(CompanyMixin, CreateView):
    model = BankStatement
    template_name = 'accounting/bank_statements/form.html'
    fields = ['bank_account', 'date_start', 'date_end', 'starting_balance', 'ending_balance', 'file_upload']
    ...
    def form_valid(self, form):
        ...
        if self.object.file_upload:
            try:
                decoded_file = self.object.file_upload.read().decode('utf-8').splitlines()
