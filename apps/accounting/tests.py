...
def test_journal_entry_service_create(company, user):
    ...
    asset_account = Account.objects.create(
        company=company, name='Asset', code='1005', account_type=Account.AccountType.ASSET
    )
    revenue_account = Account.objects.create(
        company=company, name='Rev', code='4005', account_type=Account.AccountType.REVENUE
    )
    from apps.accounting.models import Journal
    journal = Journal.objects.create(company=company, name='GEN4', code='GEN4', journal_type=Journal.JournalType.GENERAL)
...
def test_banking_service_reconcile(company, user, currency):
    ...
    bank_account = BankAccount.objects.create(
        company=company, name='Bank', account_number='1234', currency=currency,
        gl_account=Account.objects.create(company=company, name='Bank GL', code='1006', account_type=Account.AccountType.BANK)
    )
    statement = BankStatement.objects.create(company=company, bank_account=bank_account, date_start=timezone.now().date(), date_end=timezone.now().date())
    line = BankStatementLine.objects.create(statement=statement, date=timezone.now().date(), amount=Decimal('500.00'), description='Deposit')
    
    from apps.accounting.models import Journal
    journal = Journal.objects.create(company=company, name='Bank Journal', code='BNK', journal_type=Journal.JournalType.BANK)
    je = JournalEntry.objects.create(company=company, journal=journal, date=timezone.now().date())
    
    item = JournalItem.objects.create(journal_entry=je, account=bank_account.gl_account, debit=Decimal('500.00'), credit=Decimal('0.00'))
