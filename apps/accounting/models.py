...
class Journal(CompanyScoped):
    ...
    default_account = models.ForeignKey(Account, null=True, blank=True, on_delete=models.SET_NULL)
...
class JournalEntry(CompanyScoped, SequenceMixin, NotesMixin, CurrencyMixin):
    ...
    fiscal_year = models.ForeignKey('company.FiscalYear', null=True, blank=True, on_delete=models.SET_NULL)
    ...
    source_type = models.CharField(max_length=50, blank=True)  # 'invoice', 'payment', 'payroll' etc.
    source_id = models.CharField(max_length=100, blank=True)
    posted_at = models.DateTimeField(null=True, blank=True)
    posted_by = models.ForeignKey(
        'authentication.User', null=True, blank=True,
        on_delete=models.SET_NULL, related_name='posted_entries',
    )
...
class JournalItem(models.Model):
    ...
    cost_center = models.ForeignKey('CostCenter', null=True, blank=True, on_delete=models.SET_NULL, related_name='journal_items')
...
class BankTransaction(CompanyScoped, SequenceMixin):
    ...
    journal_item = models.ForeignKey(JournalItem, null=True, blank=True, on_delete=models.SET_NULL)
...
class BankStatementLine(models.Model):
    ...
    date = models.DateField(db_index=True)
    description = models.CharField(max_length=500)
    amount = models.DecimalField(max_digits=20, decimal_places=2)  # + for deposit, - for withdrawal
    reference = models.CharField(max_length=200, blank=True)
    is_reconciled = models.BooleanField(default=False, db_index=True)
    journal_item = models.OneToOneField(JournalItem, null=True, blank=True, on_delete=models.SET_NULL, related_name='bank_statement_line')
...
class CostCenter(CompanyScoped):
    ...
    parent = models.ForeignKey('self', null=True, blank=True, on_delete=models.SET_NULL, related_name='children')
    manager = models.ForeignKey('authentication.User', null=True, blank=True, on_delete=models.SET_NULL, related_name='managed_cost_centers')
    description = models.TextField(blank=True)
    is_active = models.BooleanField(default=True)
...
class Budget(CompanyScoped):
    ...
    name = models.CharField(max_length=200)
    cost_center = models.ForeignKey(CostCenter, null=True, blank=True, on_delete=models.CASCADE, related_name='budgets')
    period_start = models.DateField()
    period_end = models.DateField()
    status = models.CharField(max_length=15, choices=Status.choices, default=Status.DRAFT)
