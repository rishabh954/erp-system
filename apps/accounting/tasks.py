...
def generate_monthly_reports(company_id):
    ...
    revenue_accounts = Account.objects.filter(
        company=company, account_type=Account.AccountType.REVENUE
    )
    expense_accounts = Account.objects.filter(
        company=company, account_type=Account.AccountType.EXPENSE
    )

    total_revenue = sum(
        acc.get_balance(from_date=first_of_last_month, to_date=last_month_end)
    ...
    recipients = User.objects.filter(
        companies=company, is_active=True, is_superuser=True
...
def check_overdue_payments():
    ...
    pos = PurchaseOrder.objects.filter(balance_due__gt=0).exclude(
        status__in=[PurchaseOrder.Status.DRAFT, PurchaseOrder.Status.CANCELLED]
    )

    for po in pos:
        due_date = po.expected_delivery or (
            po.order_date + timedelta(days=po.payment_terms)
        )
        if due_date < today:
