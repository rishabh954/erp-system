...
class ChartOfAccountsView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        qs = (
            Account.objects.filter(company=self.company(), is_deleted=False)
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["totals"] = {
            ...
            ).aggregate(bal=Sum("current_balance"))["bal"]
            or 0
...
class AccountDetailView(CompanyMixin, DetailView):
    ...
    def get_object(self):
        return get_object_or_404(
            Account, pk=self.kwargs["pk"], company=self.company(), is_deleted=False
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        from_date = self.request.GET.get(
            "from_date", (timezone.localdate().replace(day=1)).isoformat()
        )
    ...
    def get(self, request):
        ...
        return render(
            ...
            self.template_name,
            {
                ...
                "currencies": __import__(
                    "apps.company.models", fromlist=["Currency"]
...
class JournalListView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        qs = (
            JournalEntry.objects.filter(company=self.company(), is_deleted=False)
            .select_related("journal", "fiscal_year", "posted_by")
    ...
    def post(self, request):
        ...
        try:
            ...
            if not entry.is_balanced():
                messages.warning(
                    request,
                    f"Journal entry {entry.number} saved but is NOT balanced (Dr={entry.total_debit}, Cr={entry.total_credit}).",
                )
            else:
...
class JournalEntryDetailView(CompanyMixin, DetailView):
    ...
    def get_object(self):
        return get_object_or_404(
            JournalEntry, pk=self.kwargs["pk"], company=self.company(), is_deleted=False
        )
...
class PostJournalEntryView(CompanyMixin, View):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            messages.success(
                request, f"Journal entry {entry.number} posted successfully."
            )
...
class BankAccountListView(CompanyMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx["total_balance"] = (
            self.get_queryset().aggregate(t=Sum("current_balance"))["t"] or 0
        )
        return ctx
...
class ProfitAndLossView(CompanyMixin, View):
    ...
    def get(self, request):
        ...
        from_date = request.GET.get(
            "from_date", timezone.localdate().replace(month=1, day=1).isoformat()
        )
        ...
        return render(
            ...
            self.template_name,
            {
                ...
                "gross_margin": (
                    round(gross_profit / revenue * 100, 1) if revenue else 0
                ),
...
class AccountingDashboardView(CompanyMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Open AR/AP
        ar_accs = Account.objects.filter(
            company=company, account_subtype="accounts_receivable"
        )
        ap_accs = Account.objects.filter(
            company=company, account_subtype="accounts_payable"
        )

        def get_open_balance(accounts):
            items = JournalItem.objects.filter(
                account__in=accounts, journal_entry__status="posted", reconciled=False
            ).aggregate(dr=Sum("debit"), cr=Sum("credit"))
            dr = items["dr"] or Decimal("0")
            cr = items["cr"] or Decimal("0")
        ...
        pl = FinancialReportingService.get_profit_and_loss(
            company, start_date=ytd_start, end_date=ytd_end
        )
        ...
        # Recent Entries
        ctx["recent_entries"] = JournalEntry.objects.filter(
            company=company, is_deleted=False
...
class BankReconciliationView(CompanyMixin, View):
    ...
    def get(self, request):
        ...
        selected_bank = (
            bank_accounts.filter(pk=selected_bank_id).first()
            if selected_bank_id
            else bank_accounts.first()
        )
        ...
        if selected_bank:
            statement_lines = (
                BankStatementLine.objects.filter(
                    statement__bank_account=selected_bank, is_reconciled=False
                )
...
class IssueCreditNoteView(CompanyMixin, View):
    ...
    def get(self, request):
        ...
        invoices = Invoice.objects.filter(
            company=company, is_deleted=False, document_type="standard"
        )
        return render(
            request,
            "accounting/issue_credit_note.html",
            {
                "customers": customers,
                "invoices": invoices,
            },
...
class CostCenterListView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        return CostCenter.objects.filter(company=self.company()).select_related(
            "parent", "manager"
        )
...
class CostCenterDetailView(CompanyMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["recent_expenses"] = (
            ...
            .order_by("-journal_entry__date")[:50]
        )
        return ctx
...
class BudgetListView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        return Budget.objects.filter(company=self.company()).select_related(
            "cost_center"
        )
...
class BudgetDetailView(CompanyMixin, DetailView):
    ...
    def get_queryset(self):
        return (
            Budget.objects.filter(company=self.company())
...
class CostCenterCreateView(CompanyMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "accounting:cost_center_detail", kwargs={"pk": self.object.pk}
        )
...
class GeneralLedgerView(CompanyMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Group by account
        accounts = Account.objects.filter(company=company, is_deleted=False).order_by(
            "code"
        )
        ledger_data = []
                ...
                ledger_data.append(
                    {
                        ...
                        "net_change": (
                            dr - cr
                            if acc.account_type in ("asset", "expense", "cogs", "bank")
                            else cr - dr
                        ),
                    }
                )
