...
class WorkflowStep(models.Model):
    ...
    approver_type = models.CharField(max_length=20, choices=ApproverType.choices,
                       blank=True, default=ApproverType.USER)
    approver_user = models.ForeignKey('authentication.User', null=True, blank=True,
                       on_delete=models.SET_NULL,
                       related_name='workflow_approver_steps')
    approver_role = models.CharField(max_length=50, blank=True)
    approver_field = models.CharField(max_length=100, blank=True,
    ...
    condition_rules = models.JSONField(default=list,
    ...
    department   = models.ForeignKey('company.Department', null=True, blank=True,
                      on_delete=models.SET_NULL)
    branch       = models.ForeignKey('company.Branch', null=True, blank=True,
                      on_delete=models.SET_NULL)

    # ── Escalation ────────────────────────────────────────────────────────────
    escalation_enabled  = models.BooleanField(default=False)
    escalation_hours    = models.PositiveIntegerField(default=24,
                             help_text='Trigger escalation after N hours of inactivity')
    escalation_action   = models.CharField(max_length=20,
                             choices=EscalationAction.choices,
                             default=EscalationAction.NOTIFY)
    escalation_to       = models.ForeignKey('authentication.User', null=True, blank=True,
                             on_delete=models.SET_NULL,
...
class WorkflowInstance(CompanyScoped):
    ...
    current_step    = models.ForeignKey(WorkflowStep, null=True, blank=True,
                         on_delete=models.SET_NULL, related_name='active_instances')
    ...
    initiated_by    = models.ForeignKey('authentication.User', on_delete=models.PROTECT,
...
class WorkflowAction(CompanyScoped):
    ...
    # Delegation fields
    delegated_to  = models.ForeignKey('authentication.User', null=True, blank=True,
                       on_delete=models.SET_NULL,
                       related_name='workflow_delegated_to')

    # Attachment / evidence
    attachment    = models.FileField(upload_to='workflow/attachments/', null=True, blank=True)
...
class ApprovalDelegation(CompanyScoped):
    delegator   = models.ForeignKey('authentication.User', on_delete=models.CASCADE,
                     related_name='delegations_made')
    delegatee   = models.ForeignKey('authentication.User', on_delete=models.CASCADE,
    ...
    workflow    = models.ForeignKey(WorkflowDefinition, null=True, blank=True,
                     on_delete=models.SET_NULL,
...
class WorkflowEscalationLog(models.Model):
    ...
    escalated_to = models.ForeignKey('authentication.User', null=True, blank=True,
                      on_delete=models.SET_NULL)
    notes      = models.TextField(blank=True)
