...
from .models import Asset, AssetCategory, AssetMaintenance, DepreciationEntry
...
class AssetListView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        ...
        ).select_related('category', 'assigned_to', 'branch').order_by('-purchase_date')
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['total_value']   = Asset.objects.filter(
            company=c, status='active', is_deleted=False
        ).aggregate(t=Sum('current_value'))['t'] or 0
...
class AssetDetailView(CompanyMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        asset = self.object
        ctx['maintenances'] = asset.maintenances.filter(is_deleted=False).order_by('-scheduled_date')
...
class AssetCreateView(CompanyMixin, View):
    ...
    def get(self, request):
        ...
        return render(request, self.template_name, {
            ...
            'branches': Branch.objects.filter(company=c, is_active=True, is_deleted=False),
    ...
    def post(self, request):
        ...
        try:
            ...
            asset.number = BaseService.generate_sequence_number('AST', Asset, company.pk)
            if request.FILES.get('image'):
                asset.image = request.FILES['image']
            asset.save()
            messages.success(request, f'Asset {asset.number} — {asset.name} registered.')
...
class ScheduleMaintenanceView(CompanyMixin, View):
    def post(self, request, pk):
        asset = get_object_or_404(Asset, pk=pk, company=self.company(), is_deleted=False)
        from decimal import Decimal
        try:
            ...
            maint.number = BaseService.generate_sequence_number('MNT', AssetMaintenance, self.company().pk)
            maint.save()
