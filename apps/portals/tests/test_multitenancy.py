...
from core.factories import UserFactory, CompanyFactory
...
def customer_b(company_b):
    ...
    return Customer.objects.create(company=company_b, name='Customer B', portal_user=user)

@pytest.fixture
def vendor_a(company):
    user = UserFactory(email='vendor_a@test.com', primary_company=company)
    return Vendor.objects.create(company=company, name='Vendor A', portal_user=user, email='vendor_a@test.com')

@pytest.fixture
def vendor_b(company_b):
    user = UserFactory(email='vendor_b@test.com', primary_company=company_b)
    return Vendor.objects.create(company=company_b, name='Vendor B', portal_user=user, email='vendor_b@test.com')
