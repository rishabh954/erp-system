...
from apps.sales.models import SalesOrder, Invoice, Payment as SalesPayment, Shipment
from apps.purchase.models import PurchaseOrder, Bill, Payment as PurchasePayment
...
from apps.crm.models import Customer, Contract
...
class CustomerPaymentListView(CustomerPortalMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['payments'] = SalesPayment.objects.filter(customer=self.get_customer()).order_by('-date')
        return ctx
    ...
    def post(self, request, pk):
        ...
        if content:
            TicketReply.objects.create(
                company=ticket.company, ticket=ticket, author=request.user, content=content, is_internal=False,
            )
...
class CustomerTicketCreateView(CustomerPortalMixin, View):
    ...
    def post(self, request):
        ...
        ticket = Ticket.objects.create(
            company=cust.company, customer=cust, title=title, description=description, priority=priority,
            category=category, source=Ticket.Source.PORTAL, status=Ticket.Status.OPEN,
        )
...
class CustomerDocumentListView(CustomerPortalMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['documents'] = Document.objects.filter(company=self.get_customer().company).order_by('-created_at')[:20]
        return ctx
...
class CustomerContractListView(CustomerPortalMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['contracts'] = Contract.objects.filter(customer=self.get_customer()).order_by('-start_date')
        return ctx
...
class CustomerShipmentListView(CustomerPortalMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['shipments'] = Shipment.objects.filter(sales_order__in=orders).order_by('-date')
        return ctx
...
class CustomerShipmentDetailView(CustomerPortalMixin, View):
    ...
    def get(self, request, pk):
        ...
        return render(request, self.template_name, {'shipment': shipment, 'customer': cust})
...
class VendorOrderListView(VendorPortalMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['orders'] = PurchaseOrder.objects.filter(vendor=self.get_vendor()).order_by('-date')
        return ctx
...
class VendorPaymentListView(VendorPortalMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['payments'] = PurchasePayment.objects.filter(vendor=self.get_vendor()).order_by('-date')
        return ctx
