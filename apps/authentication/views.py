...
from .models import User, PasswordResetToken, EmailVerificationToken, ActivityLog, UserSession
from .forms import (
        ...
        if user is None:
            ...
            try:
                ...
                if u.failed_login_attempts >= 5:
                    ...
                    messages.error(request, _('Account locked for 30 minutes due to multiple failed attempts.'))
                u.save(update_fields=['failed_login_attempts', 'locked_until'])
            except User.DoesNotExist:
                pass
            ActivityLog.objects.create(action='failed_login', description=f'Failed login for {email}')
...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        messages.success(request, 'Account created! An administrator must approve your account before you can log in. Please check your email to verify your address.')
        return redirect('auth:login')
...
class PasswordResetRequestView(View):
    ...
    def post(self, request):
        ...
        messages.success(request, _('If an account with that email exists, a reset link has been sent.'))
        return redirect('auth:login')
...
class PasswordResetConfirmView(View):
    ...
    def get(self, request, token):
        ...
        if not record.is_valid:
            messages.error(request, _('This reset link has expired or already been used.'))
...
class ProfileUpdateView(LoginRequiredMixin, View):
    def post(self, request):
        ...
        if form.is_valid():
            ...
            ActivityLog.objects.create(
                user=request.user, company=request.user.primary_company,
                action='profile_update', module='auth',
            )
...
class ActivityLogView(LoginRequiredMixin, View):
    def get(self, request):
        logs = ActivityLog.objects.filter(
            ...
            'action', 'module', 'description', 'ip_address', 'created_at'
