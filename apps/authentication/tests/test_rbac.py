...
class RBACTestCase(TestCase):
    ...
    def test_employee_cannot_deactivate_user(self):
        self.client.force_authenticate(user=self.employee)
        response = self.client.post(f"/api/v1/auth/users/{self.admin.id}/toggle_active/")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_employee_cannot_create_role(self):
        self.client.force_authenticate(user=self.employee)
        response = self.client.post("/api/v1/auth/roles/", {"name": "Hacker Role", "code": "hacker"})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_employee_cannot_update_module_permission(self):
        self.client.force_authenticate(user=self.employee)
        response = self.client.patch(f"/api/v1/auth/module-permissions/{self.mod_perm.id}/", {"can_create": True})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        
    def test_admin_cannot_update_module_permission(self):
        self.client.force_authenticate(user=self.admin)
        response = self.client.patch(f"/api/v1/auth/module-permissions/{self.mod_perm.id}/", {"can_create": True})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN) # Only superusers can edit module permissions
