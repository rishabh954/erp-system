...
from apps.authentication.models import IPRestriction, LoginHistory, UserSession
...
class TwoFactorSetupView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        totp_uri = pyotp.totp.TOTP(user.totp_secret).provisioning_uri(
            name=user.email, issuer_name="ERP System"
        )
        ...
        return render(
            request, self.template_name, {"svg": svg, "secret": user.totp_secret}
        )
    ...
    def post(self, request):
        ...
        if ip:
            IPRestriction.objects.create(
                user=request.user, ip_address=ip, description=desc, is_allowed=True
            )
