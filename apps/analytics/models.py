...
class SavedReport(models.Model):
    ...
    filters = models.JSONField(default=dict, help_text='Dict of filter key->value pairs')

    # Sorting — field name, prefix with - for descending
    sort_by = models.CharField(max_length=100, blank=True, default='-created_at')

    # Chart config
    chart_type = models.CharField(max_length=20, choices=ChartType.choices, default=ChartType.NONE)
    chart_group_by = models.CharField(max_length=100, blank=True, help_text='Field to group/label chart by')
    chart_value_field = models.CharField(max_length=100, blank=True, help_text='Field for chart values')
    ...
    is_public = models.BooleanField(default=False, help_text='Visible to all users in the company')
...
class ReportExecution(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    report = models.ForeignKey(SavedReport, on_delete=models.SET_NULL, null=True, related_name='executions')
    schedule = models.ForeignKey(ScheduledReport, on_delete=models.SET_NULL, null=True, blank=True)
