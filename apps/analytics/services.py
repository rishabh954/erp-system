...
def export_excel(
    ...
    for col_idx, header in enumerate(headers, 1):
        cell = ws.cell(
            row=1,
            column=col_idx,
            value=header.replace("__", " > ").replace("_", " ").title(),
        )
    ...
    for row_idx, row in enumerate(data, 2):
        row_fill = (
            PatternFill(start_color="F0F4FA", end_color="F0F4FA", fill_type="solid")
            if row_idx % 2 == 0
            else None
        )
    ...
    ws.append([])
    ws.cell(row=len(data) + 3, column=1, value=f"Total Records: {len(data)}").font = (
        Font(bold=True)
    )
    ws.cell(
        row=len(data) + 3,
...
def export_pdf(
    ...
    try:
        ...
        if data:
            headers = list(data[0].keys())
            table_data = [
                [h.replace("__", " > ").replace("_", " ").title() for h in headers]
            ]
...
def get_pivot_data(
    ...
    for r_key, c_dict in pivot.items():
        ...
        for i, c_key in enumerate(col_headers):
            ...
            col_totals[i] += v
            grand_total += v
        rows.append({"label": r_key, "values": row_vals, "total": row_total})
