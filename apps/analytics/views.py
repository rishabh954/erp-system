...
from .models import SavedReport, ScheduledReport, ReportExecution, CustomReport
from .services import get_data, export_csv, export_excel, export_pdf, get_pivot_data
...
class QuickExportView(ReportsMixin, View):
    ...
    def get(self, request, fmt):
        ...
        if fmt == 'csv':
            ...
            return export_excel(data, f"{filename}.xlsx", sheet_name=module.replace('_', ' ').title())
        elif fmt == 'pdf':
            return export_pdf(data, report_name=module.replace('_', ' ').title(), filename=f"{filename}.pdf")
        return HttpResponse("Invalid format", status=400)
...
class ModuleFieldsAPIView(ReportsMixin, View):
    ...
    FIELD_HINTS = {
        'sales_orders': ['number', 'status', 'total', 'customer__name', 'order_date', 'delivery_date'],
        'invoices': ['number', 'status', 'total', 'balance_due', 'customer__name', 'invoice_date', 'due_date'],
        'customers': ['name', 'phone', 'email', 'city', 'country', 'created_at'],
        'leads': ['name', 'status', 'source', 'expected_value', 'assigned_to__email', 'created_at'],
        'purchase_orders': ['number', 'status', 'total', 'vendor__name', 'order_date'],
        'purchase_invoices': ['number', 'status', 'total', 'vendor__name', 'bill_date', 'due_date'],
        'vendors': ['name', 'phone', 'email', 'city', 'country'],
        'inventory': ['product__name', 'product__sku', 'warehouse__name', 'quantity', 'average_cost'],
        'products': ['name', 'sku', 'category__name', 'standard_price', 'product_type', 'is_active'],
        'employees': ['first_name', 'last_name', 'employee_id', 'department__name', 'designation', 'status'],
        'payroll': ['employee__first_name', 'employee__last_name', 'gross_salary', 'net_salary', 'status'],
        'attendance': ['employee__first_name', 'employee__last_name', 'date', 'status', 'check_in', 'check_out'],
        'leave_requests': ['employee__first_name', 'leave_type__name', 'start_date', 'end_date', 'status'],
        'journal_entries': ['number', 'date', 'status', 'reference', 'narration'],
        'manufacturing_orders': ['number', 'status', 'quantity_to_produce', 'quantity_produced', 'planned_start_date'],
        'projects': ['name', 'number', 'status', 'manager__email', 'budget', 'actual_cost', 'start_date', 'end_date'],
        'tasks': ['title', 'status', 'priority', 'project__name', 'assigned_to__email', 'due_date'],
        'timesheets': ['user__email', 'task__title', 'date', 'hours', 'is_billable'],
        'helpdesk_tickets': ['number', 'subject', 'status', 'priority', 'customer__name', 'assigned_to__email'],
        'assets': ['name', 'asset_code', 'category__name', 'status', 'purchase_value', 'current_value'],
    }
...
class ScheduleReportView(ReportsMixin, View):
    ...
    def post(self, request, pk):
        ...
        messages.success(request, f"Report scheduled {schedule.frequency}! Will email: {schedule.recipients}")
...
class AnalyticsDashboardView(ReportsMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['saved_reports'] = SavedReport.objects.filter(created_by=user).order_by('-updated_at')[:8]
        ctx['recent_executions'] = ReportExecution.objects.filter(
            triggered_by=user
        ).select_related('report').order_by('-started_at')[:10]
        ctx['modules'] = SavedReport.Module.choices
...
class GenerateReportAPIView(ReportsMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        try:
            qs = ModelClass.objects.all().values(report.group_by_field).annotate(value=agg_func).order_by(report.group_by_field)
            labels = [str(r[report.group_by_field]) or 'Unknown' for r in qs]
            values = [float(r['value'] or 0) for r in qs]
            return JsonResponse({'labels': labels, 'values': values, 'chart_type': report.chart_type, 'name': report.name})
