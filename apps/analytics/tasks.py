...
def run_scheduled_report(self, schedule_id: str):
    ...
    from .services import get_data, export_csv, export_excel, export_pdf
    import io

    try:
        schedule = ScheduledReport.objects.select_related('report__created_by').get(id=schedule_id)
    ...
    try:
        ...
        body = schedule.body or f"Please find the attached report: {report.name}\n\nGenerated: {timezone.now().strftime('%Y-%m-%d %H:%M')}"
