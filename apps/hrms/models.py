...
class Employee(BranchScoped, AddressMixin, ContactMixin):
    ...
    # Work
    job_title = models.ForeignKey('administration.Designation', null=True, blank=True, on_delete=models.SET_NULL)
...
class LeaveRequest(CompanyScoped, SequenceMixin):
    ...
    day_type = models.CharField(max_length=20, choices=DayType.choices, default=DayType.FULL)
...
class SalaryComponent(CompanyScoped):
    ...
    calc_type = models.CharField(max_length=15, choices=CalcType.choices, default=CalcType.FIXED)
...
class PayrollPeriod(CompanyScoped, SequenceMixin):
    ...
    currency = models.ForeignKey('company.Currency', on_delete=models.PROTECT, null=True)
    notes = models.TextField(blank=True)
...
class JobPosting(CompanyScoped):
    ...
    department = models.ForeignKey('company.Department', null=True, blank=True, on_delete=models.SET_NULL, related_name='job_postings')
...
class Interview(CompanyScoped):
    ...
    interviewer = models.ForeignKey(Employee, on_delete=models.PROTECT, related_name='conducted_interviews')
...
class PerformanceAppraisal(CompanyScoped):
    ...
    manager = models.ForeignKey(Employee, null=True, blank=True, on_delete=models.SET_NULL, related_name='managed_appraisals')
    period_start = models.DateField()
    period_end = models.DateField()
    ...
    rating = models.DecimalField(max_digits=3, decimal_places=1, default=0, help_text=_("Rating from 1.0 to 5.0"))
    status = models.CharField(max_length=20, choices=[('draft', 'Draft'), ('in_progress', 'In Progress'), ('completed', 'Completed')], default='draft')
...
class TrainingProgram(CompanyScoped):
    ...
    attendees = models.ManyToManyField(Employee, related_name='training_programs', blank=True)
