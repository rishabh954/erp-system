...
from .models import (
    ...
    PayrollPeriod, Payslip, JobTitle, EmployeeDocument, EmployeeSkill, ExperienceRecord,
...
class EmployeeDetailView(CompanyMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['salary_structures'] = SalaryStructure.objects.filter(company=self.company(), is_active=True)
...
class EmployeeDocumentDeleteView(CompanyMixin, View):
    def post(self, request, pk, doc_id):
        emp = get_object_or_404(Employee, pk=pk, company=self.company())
        doc = get_object_or_404(EmployeeDocument, pk=doc_id, employee=emp, company=self.company())
        doc.delete()
...
class SalaryStructureDetailView(CompanyMixin, DetailView):
    ...
    def get_object(self):
        return get_object_or_404(SalaryStructure, pk=self.kwargs['pk'], company=self.company())
...
class SalaryComponentDeleteView(CompanyMixin, View):
    def post(self, request, pk, comp_id):
        structure = get_object_or_404(SalaryStructure, pk=pk, company=self.company())
        comp = get_object_or_404(SalaryComponent, pk=comp_id, salary_structure=structure, company=self.company())
        comp.delete()
...
class JobPostingListView(CompanyMixin, ListView):
    ...
    def post(self, request):
        ...
        if data.get('department'):
            dept = get_object_or_404(Department, pk=data.get('department'), company=self.company())
...
class JobApplicationListView(CompanyMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['jobs'] = JobPosting.objects.filter(company=self.company(), status=JobPosting.Status.PUBLISHED)
        ctx['stages'] = JobApplication.Stage.choices
        return ctx

    def post(self, request):
        data = request.POST
        job = get_object_or_404(JobPosting, pk=data.get('job_posting'), company=self.company())
        app = JobApplication(
            company=self.company(),
            job_posting=job,
...
class JobApplicationDetailView(CompanyMixin, DetailView):
    ...
    def post(self, request, pk):
        ...
        if action == 'update_stage':
            ...
            app.save()
            messages.success(request, f'Application stage updated to {app.get_stage_display()}')
...
class PerformanceAppraisalListView(CompanyMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['employees'] = Employee.objects.filter(company=self.company(), status='active', is_deleted=False)
        return ctx

    def post(self, request):
        data = request.POST
        emp = get_object_or_404(Employee, pk=data.get('employee'), company=self.company())
        manager = None
        if data.get('manager'):
            manager = get_object_or_404(Employee, pk=data.get('manager'), company=self.company())
...
class PerformanceAppraisalDetailView(CompanyMixin, DetailView):
    ...
    def post(self, request, pk):
        appraisal = get_object_or_404(PerformanceAppraisal, pk=pk, company=self.company())
        action = request.POST.get('action')
        ...
        if action == 'update_appraisal':
            ...
            appraisal.manager_feedback = request.POST.get('manager_feedback', appraisal.manager_feedback)
            appraisal.employee_feedback = request.POST.get('employee_feedback', appraisal.employee_feedback)
            appraisal.save()
...
class ExpenseClaimListView(CompanyMixin, ListView):
    ...
    def get_queryset(self):
        return ExpenseClaim.objects.filter(company=self.company()).select_related('employee', 'currency')
...
class ExpenseClaimDetailView(CompanyMixin, DetailView):
    ...
    def get_queryset(self):
        return ExpenseClaim.objects.filter(company=self.company()).select_related('employee', 'approved_by', 'currency')
    ...
    def post(self, request, pk):
        ...
        if action == 'update_status':
            ...
            if new_status in dict(ExpenseClaim.Status.choices):
                ...
                elif new_status in [ExpenseClaim.Status.REJECTED, ExpenseClaim.Status.DRAFT]:
