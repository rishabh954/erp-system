...
from apps.hrms.models import Employee, LeaveType, LeaveRequest, SalaryStructure, PayrollPeriod, JobPosting, JobApplication, PerformanceAppraisal
from apps.hrms.views import LeaveApproveView
...
def test_employee_views(client, user, employee, company):
    ...
    response = client.post(reverse('hrms:employee_document_create', args=[employee.pk]), {
        'document_type': 'resume', 'title': 'Resume', 'file': f
    })
    assert response.status_code == 302
    
    # Skill Create
    response = client.post(reverse('hrms:employee_skill_create', args=[employee.pk]), {
        'skill_name': 'Python', 'proficiency': 'expert'
    })
    assert response.status_code == 302
    
    # Experience Create
    response = client.post(reverse('hrms:experience_record_create', args=[employee.pk]), {
        'company_name': 'Acme', 'job_title': 'Dev', 'start_date': '2020-01-01', 'is_current': 'on'
    })
    assert response.status_code == 302
...
def test_leave_views(client, user, employee, company, rf):
    ...
    leave_type = LeaveType.objects.create(company=company, name='Annual', code='AN', days_allowed=10)
...
def test_salary_structure_views(client, user, company):
    ...
    # Create
    response = client.post(reverse('hrms:salary_structure_create'), {'name': 'Test Struct', 'is_active': 'on'})
    assert response.status_code == 302
    ...
    response = client.post(reverse('hrms:salary_component_create', args=[struct.pk]), data)
    assert response.status_code == 302
