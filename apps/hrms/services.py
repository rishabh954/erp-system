    ...
    def onboard_employee(self, data, user, files=None):
        ...
        if (
            ...
            and Employee.objects.filter(
                company=self.company, employee_id=employee_id
            ).exists()
        ):
            raise ValueError(
...
class LeaveService(BaseService):
    ...
    def request_leave(self, employee, data, files=None):
        ...
        leave.number = BaseService.generate_sequence_number(
            "LV", LeaveRequest, self.company.pk
        )
    ...
    def process_payroll(self, period):
        ...
        for emp in employees:
            ...
            if not payslip.number:
                ...
                payslip.number = (
                    f"{prefix}-{period.period_start.strftime('%Y%m')}-{emp.employee_id}"
                )
