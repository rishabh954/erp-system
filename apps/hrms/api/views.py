...
class EmployeeViewSet(TenantScopedViewSetMixin, viewsets.ModelViewSet):
    ...
    def check_out(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {"error": "Not checked in today"}, status=status.HTTP_400_BAD_REQUEST
            )

        if attendance.check_out:
            return Response(
                {"error": "Already checked out"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def leave_balance(self, request, pk=None):
        employee = self.get_object()
        balances = employee.leave_balances.all().values(
            "leave_type__name", "allocated", "used", "pending", "available"
        )
...
class PayrollPeriodViewSet(TenantScopedViewSetMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def process(self, request, pk=None):
        ...
        if period.status != PayrollPeriod.Status.DRAFT:
            return Response(
                {"error": "Can only process draft periods"},
                status=status.HTTP_400_BAD_REQUEST,
            )
