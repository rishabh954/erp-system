...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        company, created = Company.objects.get_or_create(
            name=company_name, defaults={"status": "active"}
        )
        ...
        uom_pcs, _ = UnitOfMeasure.objects.get_or_create(
            company=company, name="Pieces", defaults={"abbreviation": "pcs"}
        )
        ...
        for idx, fg in enumerate(finished_goods):
            bom, created = BillOfMaterial.objects.get_or_create(
                company=company, product=fg, defaults={"quantity": Decimal("1.00")}
            )
            if created:
        ...
        cat_support, _ = TicketCategory.objects.get_or_create(
            company=company, name="General Support", defaults={"sla_hours": 24}
        )
        for i in range(12):
