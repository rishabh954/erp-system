...
def determine_action(class_name):
    ...
    if "create" in class_name or "new" in class_name:
        ...
        return "delete"
    elif any(kw in class_name for kw in ["approve", "reject", "process", "send", "convert", "generate", "record", "pay", "cancel", "confirm", "complete"]):
        return "approve"
    else:
...
def process_file(filepath, app_name):
    ...
    # Replace base mixins
    content = content.replace("class CompanyScopedMixin(LoginRequiredMixin):", "class CompanyScopedMixin(PermissionRequiredMixin):")
    content = content.replace("class CompanyMixin(LoginRequiredMixin):", "class CompanyMixin(PermissionRequiredMixin):")
    ...
    for i, line in enumerate(lines):
        ...
        match = re.match(r'^(\s*)class (\w+)(?:View|APIView|ViewSet|API|List|Detail)?\((.*?)\):', line)
        if match:
            ...
            if "Mixin" in class_name or "Form" in class_name or "Filter" in class_name or "Serializer" in class_name:
                continue
                
            # Check if next lines already have required_permission
            already_has_perm = False
            for j in range(i+1, min(i+5, len(lines))):
                if "required_permission" in lines[j] or "def " in lines[j] or "class " in lines[j]:
